	...
	public List<Object[]> queryStockstatistics(String goodsName){
		try {
			String queryString = "select d.id,d.name,d.current_stock,d.current_balance,ifnull(c.zrk,0),ifnull(e.yfh,0),ifnull(f.dlye,0)"
					+ " from (select id, name,current_stock,current_balance from goods) d left outer join "
					+ " (SELECT goods, sum(stock_change) as zrk FROM stock_blotter a group by goods) c on d.id=c.goods left outer join "
					+ " (SELECT sum(a.quantity) as yfh,a.goods FROM delivery_item a inner join  delivery_note b on a.delivery_note=b.id where b.status='DELIVERY' group by a.goods) e "
