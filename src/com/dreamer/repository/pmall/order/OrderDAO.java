...
public class OrderDAO extends HibernateBaseDAO<Order> {
    ...
    public List<Object[]> getOrdersItemCount(){
        String sql="select goods.`name` as goods_name,count(item.quantity) as count from pointsmall_order_item as item left join mall_goods as goods on item.goods_id=goods.id left join pointsmall_order as po on item.order_id=po.id   where po.status=? and  po.payment_status=?  GROUP BY goods.id;";
