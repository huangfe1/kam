...
//				}
	...
	public String records(
			...
			@RequestParam(required=false) Optional<String> applyAgent,@RequestParam(required=false) Optional<String> acceptAgent,
			HttpServletRequest request, Model model) {
		try {
			User user = (User) WebUtil.getCurrentUser(request);		
            ...
            if(acceptAgent!=null&&acceptAgent.isPresent()){
                model.addAttribute("acceptAgent", acceptAgent.get());
            }
			if(user.isAdmin()){
				List<Transfer> ts=transferDAO.searchEntityByPage(parameter, null, null,null, applyAgent,acceptAgent);
				WebUtil.turnPage(parameter, request);
				...
				List<Transfer> ts=transferDAO.searchEntityByPage(parameter, null, null, agentId,applyAgent,acceptAgent);
				WebUtil.turnPage(parameter, request);
				model.addAttribute("ts", ts);
			}
			model.addAttribute("from", user.getId());
	...
	public String confirm(
		...
		try {
			...
			if(user.isAdmin()){
				...
				List<Transfer> ts=transferDAO.searchEntityByPage(parameter, null, null,null, applyAgent,null);
				WebUtil.turnPage(parameter, request);
