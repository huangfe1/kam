...
public class PmallShopCartController {
    ...
    @RequestMapping(value = "/payByAdvance.json", method = RequestMethod.POST)
    public Message payByAdvance(@ModelAttribute("parameter") Order order,
                                Address address,
                                boolean isAdd,
        ...
        try {
            ...
            //加入详细地址
            order.setShippingAddress(address.getProvince() + address.getCity() + address.getCountry() + address.getAddress());
            Agent agent1 = agentDAO.findByAgentCode(address.getConsigneeCode());
            if (Objects.isNull(agent1)) {
                throw new ApplicationException("收货人不存在");
            }
            order.setConsignee(agent1.getRealName());
            if (isAdd) {//如果添加地址
                address.setAgent(agent);
                agent.getAddresses().add(address);
            }
            order = orderCommitHandler
                    .commitOrder(order, agent, cart.buildOrderItems());
            if (order.getTotalMoney() == 0) {//应该支付的现金为0，直接扣减置换券
                orderPayHandler.pay(order, PaymentWay.Ad);
            } else {//执行微信支付
