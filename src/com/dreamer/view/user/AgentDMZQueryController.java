...
public class AgentDMZQueryController {
	...
	public void qrCodeForDomain(
		...
		try {
			...
			gp.dispose();
			//背景缓冲图
			BufferedImage bg = new BufferedImage(700, 400, BufferedImage.TYPE_INT_ARGB);
			//背景图
			ImageIcon ic=new ImageIcon(request.getSession().getServletContext().getRealPath("/assets")+File.separator+"images"+File.separator+"tg.jpg");
			
			Graphics2D gbg=(Graphics2D) bg.getGraphics();
			gbg.drawImage(ic.getImage(),0,0,null);
			gbg.drawImage(tag,32, 33, null);//二维码
...
//			gbg.drawString(parameter.getEntity().getRealName(), 290-parameter.getEntity().getRealName().length()*50, 145);
			gbg.setFont(new Font("小宋", Font.BOLD, 30));
			gbg.drawString(parameter.getEntity().getRealName(), 450, 140);
			gbg.drawString(parameter.getEntity().getAgentCode(),450,210);
