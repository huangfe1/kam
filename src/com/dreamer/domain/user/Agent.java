...
//		System.out.println(getGoodsAccounts().stream());
...
//	}
	...
	public Price caculatePrice(Goods goods, Integer quantity) {
		...
		System.out.println(gac.getAgentLevel().getName());
	...
	public Accounts generateAccounts() {
		...
		accounts.setAdvanceBalance(0d);//设置置换券
	...
	@Override
	public boolean isVip() {
        GoodsAccount gc= getGoodsAccounts().stream().filter(g->(g.getGoods().isMainGoods()&&g.getAgentLevel().getName().contains("联合股东")
				)).findFirst().orElse(null);
			return !Objects.isNull(gc);
	}
